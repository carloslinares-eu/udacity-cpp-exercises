...
class Student {
 public:
  ...
  // accessors
  [[nodiscard]] std::basic_string<char> get_name() const {return name;}
  [[nodiscard]] int get_grade () const {return grade;}
  [[nodiscard]] float get_GPA () const {return GPA;}
...
int main() {
    ...
    bool caught{false};
    try{
        Student invalid_student("Karl Linares", 5, 3.9)
    }
    catch (...) {
        caught = true;
    }
